...
test('renderer shell keeps update controls in the shared desktop status footer', () => {
  ...
  assert.match(app, /import DesktopStatusFooter from '\.\/components\/DesktopStatusFooter\.vue'/)
  ...
  assert.match(app, /<DesktopStatusFooter[\s\S]*:presentation="desktopUpdate"[\s\S]*@update="onDesktopUpdateAction"/)
  ...
  assert.match(app, /function onDesktopUpdateAction\(/)
  ...
  assert.match(app, /:collapsed="!activeScript && railWidth < 100"/)
  ...
  const footerIndex = app.indexOf('<DesktopStatusFooter')
  const sidebarStart = app.indexOf('<aside v-if="activeScript" class="sidebar">')
  assert.ok(footerIndex >= 0 && footerIndex < sidebarStart)

...
export function createDevCsBridge() {
  ...
  return {
    ...
    listAutomations: (query = {}) => apiCall('GET', `/automations?${queryString(query)}`),
    createAutomation: (payload) => apiCall('POST', '/automations', payload || {}),
    getAutomation: (uid) => apiCall('GET', `/automations/${encodePathPart(uid)}`),
    updateAutomation: (uid, payload) => apiCall('PATCH', `/automations/${encodePathPart(uid)}`, payload || {}),
    archiveAutomation: (uid) => apiCall('DELETE', `/automations/${encodePathPart(uid)}`),
    pauseAutomation: (uid) => apiCall('POST', `/automations/${encodePathPart(uid)}/pause`, {}),
    resumeAutomation: (uid) => apiCall('POST', `/automations/${encodePathPart(uid)}/resume`, {}),
    runAutomationNow: (uid, requestUid = '') => apiCall(
      'POST',
      `/automations/${encodePathPart(uid)}/run-now${requestUid ? `?request_uid=${encodePathPart(requestUid)}` : ''}`,
      {},
    ),
    listAutomationRuns: (uid, limit = 20) => apiCall(
      'GET',
      `/automations/${encodePathPart(uid)}/runs?limit=${encodePathPart(limit)}`,
    ),
    testAutomationProgram: (payload) => apiCall('POST', '/automations/program-test', payload || {}),
    ...
    runAiImageJob: (uid, snapshot) => apiCall('POST', `/ai-image/jobs/${encodePathPart(uid)}/run`, snapshot || null),

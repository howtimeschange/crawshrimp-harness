...
contextBridge.exposeInMainWorld('cs', {
  ...
  listAutomations: (query = {}) => invokeWithApiFallback('list-automations', [query],
    () => apiCall('GET', `/automations?${queryString(query)}`)),
  createAutomation: (payload) => invokeWithApiFallback('create-automation', [payload],
    () => apiCall('POST', '/automations', payload || {})),
  getAutomation: (uid) => invokeWithApiFallback('get-automation', [uid],
    () => apiCall('GET', `/automations/${encodePathPart(uid)}`)),
  updateAutomation: (uid, payload) => invokeWithApiFallback('update-automation', [uid, payload],
    () => apiCall('PATCH', `/automations/${encodePathPart(uid)}`, payload || {})),
  archiveAutomation: (uid) => invokeWithApiFallback('archive-automation', [uid],
    () => apiCall('DELETE', `/automations/${encodePathPart(uid)}`)),
  pauseAutomation: (uid) => invokeWithApiFallback('pause-automation', [uid],
    () => apiCall('POST', `/automations/${encodePathPart(uid)}/pause`, {})),
  resumeAutomation: (uid) => invokeWithApiFallback('resume-automation', [uid],
    () => apiCall('POST', `/automations/${encodePathPart(uid)}/resume`, {})),
  runAutomationNow: (uid, requestUid = '') => invokeWithApiFallback('run-automation-now', [uid, requestUid],
    () => apiCall('POST', `/automations/${encodePathPart(uid)}/run-now${requestUid ? `?request_uid=${encodePathPart(requestUid)}` : ''}`, {})),
  listAutomationRuns: (uid, limit = 20) => invokeWithApiFallback('list-automation-runs', [uid, limit],
    () => apiCall('GET', `/automations/${encodePathPart(uid)}/runs?limit=${encodePathPart(limit)}`)),

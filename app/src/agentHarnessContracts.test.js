...
test('rc.1 Web native approval card exposes allow-all through the current Session command seam', async () => {
  ...
  assert.match(source, /本会话完全访问（不再逐次审批）/)
  assert.doesNotMatch(source, /allowAll: "允许所有"/)
...
test('DSH Crawshrimp brand slots replace the official DeepSeek Harness wordmark', () => {
  ...
  assert.match(slots, /setInterval\(\(\) => \{ if \(!document\.hidden\) normalizeDocumentTitle\(\) \}, 5000\)/)
...
test('agent persona introduces itself as Crawshrimp agent', () => {
  ...
  assert.match(runtimeCordisSource, /crawshrimp-product-guide\/references\/introduction\.md/)
  const introduction = readFileSync(resolve(appRoot, '../integrations/deepseek-harness/skills/crawshrimp-product-guide/references/introduction.md'), 'utf8')
  assert.match(introduction, /可直接复制/)
  assert.match(introduction, /查看当前可用的抓虾脚本/)
  assert.match(introduction, /读取我上传的销售表/)
  assert.match(introduction, /先查看.*不要执行/)
  assert.match(profilePatch, /agent-presets[\s\S]*crawshrimp-standard/)
  assert.match(preset, /如何开始使用抓虾智能体/)
  assert.match(introduction, /当用户明确问[\s\S]*首句明确回答[\s\S]*我是抓虾智能体/)
  assert.match(introduction, /普通寒暄[\s\S]*不要主动输出长篇介绍[\s\S]*简短回答/)
  assert.match(introduction, /其它明确任务直接处理[\s\S]*不重复粘贴介绍/)
  assert.match(introduction, /普通寒暄[\s\S]*你好[\s\S]*需要我帮你处理什么/)

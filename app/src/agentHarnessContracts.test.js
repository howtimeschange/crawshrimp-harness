...
test('agent runtime missing model keys still opens DSH and gates composer with configuration modal', () => {
  ...
  assert.match(webView, /providerId: 'crawshrimp-deepseek-official'/)
  assert.match(webView, /openCustomProviderSettings/)
  assert.match(webView, /action: 'new-llm-provider'/)
  ...
  assert.match(app, /:focus-action="focusSettingsAction"/)
  assert.match(app, /const panelId = target && typeof target === 'object'[\s\S]*?target\.panelId \|\| target\.id/)
  assert.match(app, /\{ \.\.\.target, panelId, nonce: Date\.now\(\) \}/)
  assert.match(app, /focusSettingsAction\.value = null/)
  assert.match(app, /nextTick\(\(\) => \{[\s\S]*?focusSettingsAction\.value = action/)
  assert.match(settings, /focusAction/)
  assert.match(settings, /queueFocusAction/)
  assert.match(settings, /pendingFocusAction/)
  assert.match(settings, /settingsLoaded/)
  assert.match(settings, /flushFocusAction/)
  assert.match(settings, /openLlmProviderModal\(action\.providerId/)

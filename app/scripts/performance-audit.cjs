...
async function run() {
 ...
 for(const view of ['scripts','task_center','ai_image','ai_video_generation','ai_video','buyer_show_workflow','local_prompt_library','files','settings','agent']) {
 ...
 await measure('switch-30-times',async()=>{const times=[];for(let i=0;i<30;i++){const r=await navigate(['agent','ai_image','ai_video'][i%3]);times.push(r.paintMs)}return{paintMs:times}})
 ...
 await measure('renderer-only-4x-switch-12',async()=>{const times=[];for(let i=0;i<12;i++)times.push((await navigate(['agent','ai_image','ai_video'][i%3])).paintMs);return{paintMs:times,note:'renderer slowdown only; not an i3 simulation'}})

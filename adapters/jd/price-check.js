;(async () => {
  ...
  const threshold = (parseFloat(params.threshold) || 50) / 100
  ...
  try {
    if (phase === 'main') {
      return nextPhase('ensure_target', 0, { page })
    }

    if (phase === 'ensure_target') {
      const shopId = parseShopId(shopUrl)
      if (!shopId) return { success: false, error: '无法解析店铺 ID，请检查店铺链接格式' }

      const searchUrl = buildSearchUrl(shopId, page, 60)
      ...
      if (n === 0) return { success: false, error: '商品列表加载超时，请确认店铺地址正确或页面可访问' }
    ...
    if (phase === 'collect') {
      ...
      return complete(violations, hasNextPage())
    }

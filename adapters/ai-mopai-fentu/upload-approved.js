(async () => {
  ...
  function indexSourceFiles(listing) {
    ...
    for (const filePath of listing.paths) {
      ...
      if (!byCode[folder.code]) {
        byCode[folder.code] = {
          folder_name: folder.folder_name, code_type: folder.code_type, files: [],
          root_files: [], upload_folders: [], structured_files: [], directories: [],
        }
      }
      const entry = byCode[folder.code]
  ...
  function indexCloudSourceFiles(files) {
    ...
    for (const rawFile of files || []) {
      ...
      if (!byCode[folder.code]) {
        byCode[folder.code] = {
          folder_name: folder.folder_name, code_type: folder.code_type, files: [],
          copy_entries: [], structured_files: [], directories: [],
        }
      }
      const entry = byCode[folder.code]
  ...
  function buildStructuredUploadPlan(job) {
    return [
  ...
  if (phase === 'main') {
    ...
    if (sourceMode === 'local') {
      ...
      built = buildJobs(rows, indexSourceFiles(listing), 'local')
    }
    let mounts
    try {
    ...
    if (sourceMode === 'cloud') {
      ...
      built = buildJobs(rows, indexCloudSourceFiles(sourceFiles), 'cloud')
    }
    const runnable = []
  ...
  if (phase === 'check_parent') {
    ...
    if (checked.folder) {
      ...
      if (hasLocalSubfolders(job)) {
        ...
        if (!structured.missing_files.length && !(structured.missing_directories || []).length && !unexpected) {
          return appendAndAdvance(job, '跳过', structured.count, '云盘已存在同名目标文件夹且相对目录结构完整；未重复上传')
        }
        return appendAndAdvance(job, '需复核', structured.count, '云盘已存在同名目标文件夹但子目录结构不完整或包含额外内容；为避免覆盖、拆散目录，未自动补传')
      }
  ...
  if (phase === 'verify_target_folder') {
    ...
    if (hasLocalSubfolders(job)) {
      const uploadPlan = buildStructuredUploadPlan(job)
  ...
  if (phase === 'copy_cloud_files') {
    ...
    if (!inspected.missing_files.length && !(inspected.missing_directories || []).length) {
      return appendAndAdvance(job, '跳过', inspected.count, '目标目录已包含完整源文件夹结构，已按相对路径及 filehash/文件大小校验；未重复复制')
    }
  ...
  if (phase === 'verify_local_structure_upload') {
    ...
    if (inspected.ok && !inspected.missing_files.length && !(inspected.missing_directories || []).length && !unexpected) {
      return appendAndAdvance(job, '上传成功', inspected.count, '本地款式文件夹已按原相对路径完整上传，子文件夹结构校验通过')
    }
    const startedAt = Number(shared.wait_started_at || Date.now())

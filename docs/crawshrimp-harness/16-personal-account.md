...
账号现用于可关闭的产品统计：登录后采集功能、任务结果、模型用量与版本元数据，由 Electron 主进程上报 Cloudflare，Supabase 存储。此前“账号功能不会自动上传任何本地数据”的描述适用于仅 Auth 的版本；统计扩展的具体数据范围、关闭入口和环境隔离见 [产品使用统计说明](19-product-analytics.md)。聊天与文件正文、密钥不上传，账号仍不授权远程执行本机任务。

...
自动化的主实体是一个持久化的智能体目标：用户说明要做什么和何时做，Harness 在到点时启动一次受控的 Agent Turn。适配器、脚本、Chrome CDP、文件和 AI 服务都是这次 Agent Turn 可调用的执行能力，而不是定时任务本身。
...
2. 每次触发都运行一个受控 Agent Turn，能调用现有 Harness 与 Crawshrimp 能力。
3. 每个自动化可选择隔离上下文或继承创建它的会话。
4. 缺少能力时，智能体可为当前运行创建、校验和使用临时脚本；临时脚本不会自动发布为共享资产。
5. 运行、产物、失败、重试、通知和人工复核都有可读回的证据。
...
~~~text
Agent 对话
  │ automation.create / update / pause / run
  ▼
Automation Controller ──持久化──> SQLite automation tables
  │                              ▲
  │ 注册、恢复、到点唤醒          │ run history and links
  ▼                              │
APScheduler                       │
  │ due automation                │
  ▼                              │
Context Resolver ──> AgentService / DSH Worker / MCP
                              │
                              ▼
            Adapter / Task Instance / CDP / Files / AI / 临时脚本
                              │
                              ▼
                    Automation Run、产物引用、通知
~~~
...
| 组件 | 负责 | 不负责 |
| --- | --- | --- |
| Automation Controller | 定义、调度注册、幂等领取、并发、重试、取消和状态投影 | 浏览器或业务动作 |
...
- schedule_kind：at、every 或 cron；对应的计划值、timezone 和计算出的 next_run_at。cron 使用五段标准表达式；every 保存首次锚点。
...
- run_uid、automation_uid、scheduled_at、started_at、finished_at、attempt、idempotency_key。
- status：queued、running、retry_scheduled、completed、failed、skipped_overlap、canceled、missed 或 needs_review。
- agent_session_id、agent_run_id、task_instance_uid、temporary_workspace_ref。
- execution_policy_snapshot、result_summary、error_code、error_message、notification_status。

idempotency_key 使用 automation_uid 与 scheduled_at 的固定组合，并由数据库唯一约束保护。无论 APScheduler 重复回调、应用重启或用户重复点击，Controller 只允许同一个计划桶有一个权威 Run。可重试错误在这个 Run 内增加 attempt 并更新状态，不创建第二个父 Run；execution_policy_snapshot 让审计能准确还原当次实际使用的授权。
...
1. 用户在 Agent 对话中表达目标与频率。Agent 补足必要参数后调用 automation.create；创建卡片回显目标、频率、时区、上下文模式、授权包和下次运行时间。
2. Controller 保存定义、注册 APScheduler，并在自动化中心显示状态。
3. 到点后 Controller 原子领取计划桶。如果已存在运行记录则直接返回；如果该 Automation 有 running Run，则新增 skipped_overlap 记录。
...
本机 Harness 未运行时没有执行者。启动时，Controller 识别错过的计划并记录 missed，不自动追赶。用户可以在自动化中心选择立即补跑。
...
对话是主入口。用户可说“每个工作日 8:30 检查店铺库存，异常发我钉钉”。Agent 应补齐缺失的频率、时区、上下文和授权信息，再创建 Automation。创建成功后返回结构化卡片和管理入口。

### 自动化中心

在任务中心增加独立的“自动化”页签，而不把它混入固定脚本任务：

- 列表：名称、目标摘要、下次运行、上下文模式、最后结果和启停状态。
- 详情：定义、授权包、计划、最近运行、Agent/Task/产物引用和错误证据。
- 操作：立即运行、暂停、恢复、编辑、复制、归档；needs_review 可扩展授权后重试。
- 频率：提供自然语言/预设的每日和每周编辑；高级编辑器支持 at、interval、cron 和显式时区。
...
- automation.runs 与 automation.retry_review。
...
从自然语言创建自动化后，用户能在自动化中心查看、暂停、立即运行和打开本次产物。一次缺失授权的运行必须展示 needs_review，扩展授权并重试后才能继续；不会出现无记录的后台动作。

## 实施边界

首版实现 Automation Controller、SQLite 存储、调度恢复、Automation MCP/API、AgentService 上下文接入、临时脚本运行域、自动化中心和覆盖上述行为的测试。云端常驻执行、跨机编排、自动脚本发布和旧 Task Schedule 迁移留在后续独立设计中。

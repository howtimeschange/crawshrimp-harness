...
- DWS 随抓虾打包，无需 npm、Homebrew 或全局安装。`dws` 已加入抓虾子进程 PATH。
- 精确二进制路径为 `CRAWSHRIMP_DWS_EXECUTABLE`；CLI 根为 `CRAWSHRIMP_CLI_ROOT`，skill 根为 `CRAWSHRIMP_SKILL_ROOT`。先调用 `skill_list` 可取得绝对路径和可用状态，不假设当前目录是安装目录。
- Bash: `"$CRAWSHRIMP_DWS_EXECUTABLE" --version`；PowerShell: `& $env:CRAWSHRIMP_DWS_EXECUTABLE --version`。后续文档中的 `dws` 均指此内置程序。
- 首次使用按 `dws auth login --help` 指引完成用户登录。预装不会登录、创建订阅或发送消息。不要读取、打印或复制凭据文件。
- 技能内 Python 脚本使用 `CRAWSHRIMP_PYTHON_EXECUTABLE`；先进入本技能目录再使用相对路径。
- 版本由抓虾统一更新，不执行 `dws upgrade`、全局安装器或向其他 Agent 目录安装 skill。
- 用户已经明确授权的同一操作范围无需重复确认；未授权的外部写入仍需用户确认。
- 上游文档和许可来源见 [UPSTREAM.md](./UPSTREAM.md)。

...
Environment:
    1XM_API_KEY or ONEXM_API_KEY - API key for 1xm.ai.
"""
...
def get_api_key():
    """Read from explicit environment variables, then fail loudly."""
    key = os.environ.get("1XM_API_KEY") or os.environ.get("ONEXM_API_KEY")
    if key:
        return key
    # No valid key found — fail clearly
    print("ERROR: 1XM_API_KEY or ONEXM_API_KEY not set. Set it via:", flush=True)
    print("  export 1XM_API_KEY=your_api_key_here", flush=True)
    raise RuntimeError("1XM_API_KEY or ONEXM_API_KEY environment variable is not set")
...
if __name__ == "__main__":
    ...
    result = generate(prompt, ref_path, model, out_path, size)
